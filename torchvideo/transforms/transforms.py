...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, vid):
        ...
        return [F.hflip(img) for img in vid]
...
class RandomVerticalFlip(object):
    ...
    def __call__(self, vid):
        ...
        return [F.vflip(img) for img in vid]
...
class RandomResizedCrop(object):
    ...
    def __call__(self, vid):
        """
        Args:
            vid (list of PIL Image): Images to be cropped and resized.

        Returns:
            list of PIL Image: Randomly cropped and resized images.
        """
        i, j, h, w = self.get_params(vid[0], self.scale, self.ratio)
        return [F.resized_crop(img, i, j, h, w, self.size, self.interpolation) for img in vid]
...
class RandomFixResizedCrop(object):
    """Crop the given PIL Image to random size and aspect ratio.
